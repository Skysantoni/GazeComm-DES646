...
WINK_AR_DIFF_THRESH = 0.04  # Increased threshold for clearer wink detection
...
# --- ADDED: On-screen notification variables ---
notification_text = ""
...
while True:
        ...
        if leftEAR < rightEAR:
            if leftEAR < EYE_AR_THRESH:
                ...
                if LEFT_WINK_COUNTER > WINK_CONSECUTIVE_FRAMES:
                    # print("LEFT CLICK") # 
                    notification_text = "LEFT CLICK" # <-- NEW
                    notification_timestamp = time.time() # <-- NEW
        ...
        # RIGHT WINK - For Right Click
            ...
            if rightEAR < EYE_AR_THRESH:
                ...
                if RIGHT_WINK_COUNTER > WINK_CONSECUTIVE_FRAMES:
                    # print("RIGHT CLICK") # <-- OLD
                    notification_text = "RIGHT CLICK" # <-- NEW
                    notification_timestamp = time.time() # <-- NEW
    ...
    # Check for BLINK (Both Eyes) - For Drag Mode
        ...
        if ear <= 0.2:
            ...
            if EYE_COUNTER > EYE_AR_CONSECUTIVE_FRAMES:
                ...
                if DRAG_MODE:
                    # print("DRAG MODE ON") # <-- OLD
                    notification_text = "DRAG MODE ON" # <-- NEW
                    notification_timestamp = time.time() # <-- NEW
                    pag.mouseDown(button='left')
                else:
                    # print("DRAG MODE OFF - Copying and Speaking") # <-- OLD
                    notification_text = "DRAG MODE OFF" # <-- NEW
                    notification_timestamp = time.time() # <-- NEW
    ...
    if mar > MOUTH_AR_THRESH:
        ...
        if MOUTH_COUNTER >= MOUTH_AR_CONSECUTIVE_FRAMES:
            ...
            # print(f"INPUT MODE: {INPUT_MODE}") # <-- OLD
            
            # --- NEW ---
            if INPUT_MODE:
                notification_text = "INPUT MODE ON"
                ANCHOR_POINT = nose_point # Set anchor only when turning on
            else:
                notification_text = "INPUT MODE OFF"
            notification_timestamp = time.time()
            # --- END NEW ---
            
            MOUTH_COUNTER = 0
            # ANCHOR_POINT = nose_point # <-- MOVED
    ...
    if INPUT_MODE:
        # cv2.putText(frame, "INPUT MODE: ON", (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, RED_COLOR, 2) # <-- REMOVED
        x, y = ANCHOR_POINT
        nx, ny = nose_point
        ...
        drag_speed = 18 # Pixels to move per frame
        if dir == 'right':
    ...
    # --- ADDED: Draw temporary notification text ---
